...
TEST(BoardTest, create)
{
    ...
    ASSERT_EQ(Chess::Color::White, board.getMoveColor());

    Chess::Board board1(8, 8, Chess::Color::Black);
    ASSERT_EQ(Chess::Color::Black, board1.getMoveColor());
}

TEST(BoardTest, changeColor)
{
    Chess::Board board(8, 8);
    ASSERT_EQ(Chess::Color::White, board.getMoveColor());

    board.setMoveColor(Chess::Color::Black);
    ASSERT_EQ(Chess::Color::Black, board.getMoveColor());
}
...
TEST(BoardTest, addFigure)
{
    ...
    board.addFigure(Chess::Rook(1, Chess::Color::White, 0, 0));
    board.addFigure(Chess::Bishop(2, Chess::Color::White, 0, 5));
    ...
    ASSERT_EQ(Chess::Color::White, figure1.getColor());
...
TEST(BoardTest, write)
{
    ...
    std::string buffer("\x00\x00\x00\x00\x00\x01\x05\x00\x0\x00" "BOARD"
            "\x00\b\x00\x00\x00\x00\b\x00\x00\x00\x00\x01\x00\x00\x00\x00\x02\x00\x00\x00\x00\x01\x00\x00\x00\x01\x01\x00\x00\x00"
            "R\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x01\x01"
            "\x00\x00\x00" "B\x00\x00\x00\x00\x00\x00\x05\x00\x00\x00\x00\x00\x00\x00\x00", 87);
    board.write(stream);
...
TEST(BoardTest, read)
{
    ...
    ASSERT_EQ(Chess::Color::White, figure2.getColor());
}
